...
class ConnectionProfile:
    ...
    def __init__(self, name: str, host: str = 'localhost', port: int = 6379,
        ...
        Args:
            ...
            ssl_verify (bool): Verify the Redis server certificate.
            ...
            mode (Optional[str]): Connection mode: standalone, cluster, or sentinel.
            sentinel_service_name (Optional[str]): Sentinel monitored master name.
            sentinels (Optional[List[Tuple[str, int]]]): Sentinel endpoints.
            read_preference (str): Sentinel read routing policy.
            replica_selector (str): Sentinel replica selection policy.
        """
        self.name = name
        self.host = host
        self.port = port
        ...
        self.mode = mode or ('cluster' if cluster_mode else 'standalone')
        ...
        self.sentinel_mode = self.mode == 'sentinel'
        ...
        if self.sentinel_mode and (
                not self.sentinel_service_name or not self.sentinels
        ):
            raise ValueError(
                'Sentinel mode requires sentinel_service_name and sentinels'
            )
...
class RedisConnection:
    ...
    def __init__(self, profile: ConnectionProfile):
        ...
        self.is_sentinel = profile.sentinel_mode
    ...
    def _connect_standalone(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.profile.ssl:
                kwargs['ssl'] = True
                kwargs['ssl_verify'] = self.profile.ssl_verify
            ...
            if self.profile.sentinel_mode:
                kwargs.update({
                    'sentinel_service_name': self.profile.sentinel_service_name,
                    'sentinels': self.profile.sentinels,
                    'read_preference': self.profile.read_preference,
                    'replica_selector': self.profile.replica_selector,
                    'sentinel_username': self.profile.sentinel_username,
                    'sentinel_password': self.profile.sentinel_password,
                    'sentinel_ssl': self.profile.sentinel_ssl,
                    'sentinel_ssl_ca_certs': self.profile.sentinel_ssl_ca_certs,
                    'sentinel_ssl_certfile': self.profile.sentinel_ssl_certfile,
                    'sentinel_ssl_keyfile': self.profile.sentinel_ssl_keyfile,
                    'sentinel_ssl_check_hostname': self.profile.sentinel_ssl_check_hostname,
                    'sentinel_ssl_verify': self.profile.sentinel_ssl_verify,
                })
            ...
            if self.profile.sentinel_mode:
                logger.info(
                    f"Connected through Sentinel to {self.client.host}:{self.client.port}"
                )
                return True, "Connected through Sentinel"
    ...
    def _connect_cluster(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not self.cluster_client.ping():
                raise RedisConnectionError('Redis Cluster PING failed')
            self.connected = True
...
class KeyListView(urwid.WidgetWrap):
    ...
    def set_keys(self, keys_with_types: List[Tuple[bytes, str]]):
        ...
        self.begin_update()
        self.append_keys(keys_with_types)
        logger.info("KeyListView.set_keys completed")

    def begin_update(self):
        """Clear the current collection before an incremental refresh."""
        self.all_keys = []
        self.filtered_keys = []
    ...
    def append_keys(self, keys_with_types: List[Tuple[bytes, str]]):
        """Append a scanned batch without rebuilding the existing widgets."""
        self.all_keys.extend(keys_with_types)
        ...
        for key, key_type in keys_with_types:
            ...
            self._append_key_widget(key, key_type)

        self.update_header()

    def _matches_filter(self, key: bytes) -> bool:
        """Return whether a key matches the active local filter."""
        ...
        return pattern in key_str

    def _append_key_widget(self, key: bytes, key_type: str):
        """Create and append one key row to the walker."""
        item = KeyListItem(
            key,
            key_type,
            on_select_callback=self.on_key_selected,
            marked=key in self.marked_keys
        )
        self.key_cache[key] = item
        self.walker.append(urwid.AttrMap(item, 'key_item', 'key_item_focus'))
...
class RedisCommanderUI:
    ...
    MAX_KEYS = 5000
    SCAN_COUNT = 1000
    TYPE_PIPELINE_BATCH_SIZE = 1000
    DB_KEY_COUNT_CACHE_TTL = 5.0
    ...
    def __init__(self, args):
        ...
        self._scan_expected_workers = 0
        self._scan_finished_workers = 0
        self._scan_seen_keys = set()
        self._scan_errors = []
        self._db_key_counts_cache = {}
        self._database_count_cache = {}
        self.interface_mode = 'profiles'
        ...
        # Создаем интерфейс
        self.create_ui()
        self.create_profile_selector_ui()
    ...
    def _parse_profiles(self, data: dict) -> Dict[str, ConnectionProfile]:
        ...
        for name, config in data.items():
            ...
            base_params = ['name', 'host', 'port', 'password', 'username',
                           ...
                           'ssl_certfile', 'ssl_keyfile', 'mode',
                           'sentinel_service_name', 'read_preference',
                           'replica_selector', 'sentinel_username',
                           'sentinel_password', 'sentinel_ssl',
                           'sentinel_ssl_ca_certs', 'sentinel_ssl_certfile',
                           'sentinel_ssl_keyfile', 'sentinel_ssl_check_hostname',
                           'sentinel_ssl_verify']
            ...
            if 'sentinels' in config:
                ...
                if isinstance(sentinels, list):
                    ...
                    for sentinel in sentinels:
                        if isinstance(sentinel, str) and ':' in sentinel:
                            host, port_str = sentinel.rsplit(':', 1)
                            try:
                                formatted_sentinels.append((host, int(port_str)))
                            except ValueError:
                                logger.warning(f"Invalid Sentinel endpoint {sentinel}")
                        elif isinstance(sentinel, dict):
                            host = sentinel.get('host')
                            port = sentinel.get('port', 26379)
                            try:
                                if host:
                                    formatted_sentinels.append((str(host), int(port)))
                            except (TypeError, ValueError):
                                logger.warning(f"Invalid Sentinel endpoint {sentinel}")
                        elif isinstance(sentinel, (list, tuple)) and len(sentinel) == 2:
                            try:
                                formatted_sentinels.append(
                                    (str(sentinel[0]), int(sentinel[1]))
                                )
                            except (TypeError, ValueError):
                                logger.warning(f"Invalid Sentinel endpoint {sentinel}")
                    if formatted_sentinels:
                        clean_config['sentinels'] = formatted_sentinels
    ...
    def validate_profiles(self):
        ...
        for name, profile in self.profiles.items():
            if profile.sentinel_mode:
                if not profile.sentinel_service_name or not profile.sentinels:
                    logger.error(
                        f"Profile '{name}' requires Sentinel service name and endpoints"
                    )
                if profile.read_preference not in (
                        'master', 'replica_preferred', 'replica_only'
                ):
                    logger.error(
                        f"Profile '{name}' has invalid read_preference: "
                        f"{profile.read_preference}"
                    )
            elif profile.cluster_mode:
    ...
    def _profile_endpoint(self, profile: ConnectionProfile) -> str:
        """Return a human-readable endpoint without exposing credentials."""
        if profile.socket_path:
            return profile.socket_path
        if profile.sentinel_mode:
            endpoints = ', '.join(
                f'{host}:{port}' for host, port in profile.sentinels
            )
            return f'{profile.sentinel_service_name} via {endpoints}'
        if profile.cluster_mode and profile.cluster_nodes:
            return ', '.join(f'{host}:{port}' for host, port in profile.cluster_nodes)
        return f'{profile.host}:{profile.port}'

    def create_profile_selector_ui(self):
        """Build the startup screen containing configured Redis profiles."""
        ...
        self.profile_walker = urwid.SimpleFocusListWalker([])
        for name, profile in self.profiles.items():
            if profile.sentinel_mode:
                mode = 'SENTINEL'
            elif profile.cluster_mode:
                ...
                mode = 'STANDALONE'
            tls = ', TLS' if profile.ssl else ''
            label = f' {profile.name}  [{mode}{tls}]  {self._profile_endpoint(profile)}'
            button = urwid.Button(
                label,
                on_press=self.on_profile_select,
                user_data=name
            )
            self.profile_walker.append(
                urwid.AttrMap(button, 'connection', 'connection_selected')
            )

        if not self.profiles:
            self.profile_walker.append(
                urwid.Text(('warning', ' No connection profiles found in the configuration.'))
            )

        self.profile_walker.append(urwid.Divider())
        exit_button = urwid.Button(' Exit', on_press=lambda button: self.exit_main())
        self.profile_walker.append(
            urwid.AttrMap(exit_button, 'button_danger', 'button_focus')
        )

        self.profile_listbox = urwid.ListBox(self.profile_walker)
        profile_panel = urwid.LineBox(
            ScrollBar(urwid.AttrMap(self.profile_listbox, 'background')),
            title='Configured servers and clusters'
        )
        self.profile_status_text = urwid.Text(
            ' Select a profile and press Enter. F10 or Q: Exit'
        )
        self.profile_frame = urwid.Frame(
            header=header,
            body=urwid.Padding(profile_panel, left=2, right=2),
            footer=urwid.AttrMap(self.profile_status_text, 'footer')
        )
        self.profile_frame.focus_position = 'body'
    ...
    def create_ui(self):
        ...
        # Левая панель: базы активного подключения вверху, ключи внизу.
        ...
        self.main_content = urwid.Pile([
            ...
            ('weight', 1, urwid.AttrMap(self.content_columns, 'background')),
        ])
    ...
    def toggle_console(self):
        ...
        if self.console_mode:
            ...
            self.main_content.contents = [
                ...
                (urwid.AttrMap(self.content_columns, 'background'), ('weight', 1)),
            ]
            self.console_mode = False
            ...
            self.main_content.contents = [
                (urwid.AttrMap(urwid.Padding(
                    self.create_toolbar(),  # ✅ Используем метод
                    left=1, right=1), 'background'), ('pack', None)),
                (urwid.AttrMap(self.content_columns, 'background'), ('weight', 2)),
    ...
    def update_connection_tree(self):
        """Display databases belonging only to the active connection."""
        ...
        self.left_panel.set_title(f'{name} - Databases & Keys')

        if conn.is_cluster:
            db_key_counts = {0: self.key_count} if self.key_count > 0 else {}
            database_count = 1
        else:
            db_key_counts = self.get_all_db_key_counts(conn)
            database_count = self.get_database_count(conn, db_key_counts)

        for db_num in range(database_count):
            is_current = conn.current_db == db_num
            db_icon = ' ▪ ' if is_current else ' ▫ '
            key_count = db_key_counts.get(db_num, 0)
            db_label = f'{db_icon}DB{db_num}'
            if key_count > 0:
                db_label += f' ({key_count} keys)'

            db_btn = urwid.Button(
                db_label,
                on_press=self.on_db_select,
    ...
    def get_all_db_key_counts(self, conn: RedisConnection) -> Dict[int, int]:
        """Return cached DB key counts from a single INFO keyspace call."""
        ...
        cache_key = id(conn)
        now = time.monotonic()
        cached = self._db_key_counts_cache.get(cache_key)
        if cached and now - cached[0] < self.DB_KEY_COUNT_CACHE_TTL:
            return cached[1].copy()

        try:
            info = conn.client.info('keyspace')
            keyspace = info.get('keyspace', info) if isinstance(info, dict) else {}

            for db_name, raw_stats in keyspace.items():
                if not isinstance(db_name, str) or not db_name.startswith('db'):
                    continue

                try:
                    db_num = int(db_name[2:])
                    if isinstance(raw_stats, dict):
                        count = int(raw_stats.get('keys', 0))
                    else:
                        fields = dict(
                            field.split('=', 1)
                            for field in str(raw_stats).split(',')
                            if '=' in field
                        )
                        count = int(fields.get('keys', 0))

                    if count > 0:
                        db_counts[db_num] = count
                except (TypeError, ValueError):
                    logger.warning(f"Failed to parse keyspace stats for {db_name}: {raw_stats}")

            self._db_key_counts_cache[cache_key] = (now, db_counts.copy())
        except Exception as e:
            logger.error(f"Failed to get DB key counts from INFO keyspace: {e}")

        return db_counts

    def _invalidate_db_key_counts(self, conn: Optional[RedisConnection] = None):
        """Invalidate cached database counts for one or all connections."""
        if conn is None:
            self._db_key_counts_cache.clear()
        else:
            self._db_key_counts_cache.pop(id(conn), None)

    def get_database_count(self, conn: RedisConnection,
                           db_counts: Optional[Dict[int, int]] = None) -> int:
        """Return the configured standalone DB count with safe fallbacks."""
        if conn.is_cluster:
            return 1

        cache_key = id(conn)
        cached = self._database_count_cache.get(cache_key)
        if cached is not None:
            return cached

        db_counts = db_counts or {}
        observed_count = max(
            [self.DEFAULT_DATABASE_COUNT, conn.current_db + 1]
            + [db_num + 1 for db_num in db_counts]
        )

        try:
            config = conn.client.config_get('databases')
            configured_count = int(config.get('databases', 0))
            database_count = max(observed_count, configured_count)
        except Exception as e:
            database_count = observed_count
            logger.warning(
                f"Failed to read configured database count; "
                f"using {database_count}: {e}"
            )

        self._database_count_cache[cache_key] = database_count
        return database_count

    def on_profile_select(self, button, profile_name: str):
        """Connect to the profile selected on the startup screen."""
        profile = self.profiles.get(profile_name)
        if not profile:
            return

        self.profile_status_text.set_text(
            ('footer', f' Connecting to {profile.name} ({self._profile_endpoint(profile)})...')
        )
        try:
            self.loop.draw_screen()
        except Exception:
            # The loop is not started yet in unit tests and during early startup.
            pass
        self.connect_to_profile(profile)

    def _redact_diagnostic_value(self, value: Any,
                                 profile: ConnectionProfile) -> str:
        """Return diagnostic text with the configured password removed."""
        text = str(value)
        if profile.password:
            text = text.replace(str(profile.password), '***')
        if profile.sentinel_password:
            text = text.replace(str(profile.sentinel_password), '***')
        return text

    def build_connection_diagnostics(self, profile: ConnectionProfile,
                                     conn: RedisConnection,
                                     message: str) -> List[str]:
        """Build detailed, secret-free diagnostics for a failed connection."""
        error = getattr(conn, 'last_error', None)
        error_type = type(error).__name__ if error else 'UnknownError'
        safe_message = self._redact_diagnostic_value(message, profile)

        lines = [
            f'Profile: {profile.name}',
            f'Mode: {profile.mode.capitalize()}',
            f'Endpoint: {self._profile_endpoint(profile)}',
            f'TLS: {"enabled" if profile.ssl else "disabled"}',
            f'TLS certificate verification: {"enabled" if profile.ssl_verify else "disabled"}',
            f'ACL username: {profile.username if profile.username else "not configured"}',
            f'Password: {"configured" if profile.password else "not configured"}',
            f'Client certificate: {"configured" if profile.ssl_certfile else "not configured"}',
            f'CA certificate: {"configured" if profile.ssl_ca_certs else "not configured"}',
            '',
            f'Exception: {error_type}',
            f'Message: {safe_message or "No error details were returned"}',
            '',
            'Suggested checks:',
        ]

        if profile.sentinel_mode:
            lines.insert(3, f'Sentinel service: {profile.sentinel_service_name}')
            lines.insert(4, f'Read preference: {profile.read_preference}')
            lines.insert(
                5,
                f'Sentinel authentication: '
                f'{"configured" if profile.sentinel_password else "not configured"}'
            )
            lines.insert(
                6,
                f'Sentinel TLS: {"enabled" if profile.sentinel_ssl else "disabled"}'
            )

        error_text = f'{error_type} {safe_message}'.lower()
        if 'refused' in error_text:
            lines.append('- Check that Redis is running, listening on this endpoint, and allowed by the firewall.')
        elif 'timeout' in error_text or 'timed out' in error_text:
            lines.append('- Check routing, firewall rules, endpoint reachability, and whether TLS is required.')
        elif any(token in error_text for token in ('auth', 'noauth', 'wrongpass')):
            lines.append('- Check the Redis ACL username, password, and permissions for this profile.')
        elif any(token in error_text for token in ('ssl', 'tls', 'certificate')):
            lines.append('- Check TLS mode, CA trust, client certificate, key, and server hostname.')
        else:
            lines.append('- Check endpoint reachability, Redis logs, credentials, and TLS settings.')

        if profile.cluster_mode:
            lines.append('- Check that a seed node is reachable and CLUSTER INFO reports cluster_state:ok.')
        elif profile.sentinel_mode:
            lines.append('- Check Sentinel quorum, monitored service name, announced addresses, and failover state.')
        return lines

    def close_connection_error(self, button=None):
        """Close the connection error dialog and restore the profile selector."""
        self.interface_mode = 'profiles'
        self.loop.widget = self.profile_frame
        self.profile_status_text.set_text(
            ' Select a profile and press Enter. F10 or Q: Exit'
        )

    def retry_connection(self, button, profile_name: str):
        """Retry a failed profile connection from the diagnostics dialog."""
        self.close_connection_error()
        self.on_profile_select(None, profile_name)

    def show_connection_error(self, profile: ConnectionProfile,
                              conn: RedisConnection, message: str):
        """Show a scrollable connection diagnostics modal."""
        diagnostics = self.build_connection_diagnostics(profile, conn, message)
        detail_walker = urwid.SimpleFocusListWalker([
            urwid.Text(line) for line in diagnostics
        ])
        detail_list = urwid.ListBox(detail_walker)

        retry_button = urwid.Button(
            'Retry',
            on_press=self.retry_connection,
            user_data=profile.name
        )
        back_button = urwid.Button(
            'Back',
            on_press=lambda button: self.close_connection_error(button)
        )
        exit_button = urwid.Button(
            'Exit',
            on_press=lambda button: self.exit_main()
        )
        buttons = urwid.Columns([
            urwid.AttrMap(retry_button, 'button', 'button_focus'),
            urwid.AttrMap(back_button, 'button', 'button_focus'),
            urwid.AttrMap(exit_button, 'button_danger', 'button_focus'),
        ], dividechars=1)
        dialog = urwid.LineBox(
            urwid.Pile([
                ('weight', 1, ScrollBar(detail_list)),
                ('pack', urwid.Divider()),
                ('pack', buttons),
            ]),
            title=f'Connection failed: {profile.name}'
        )
        overlay = urwid.Overlay(
        ...
        self.interface_mode = 'connection_error'
    ...
    def connect_to_profile(self, profile: ConnectionProfile) -> bool:
        """Connect to a profile and open the main interface on success.
        ...
        if success:
            ...
            if profile.cluster_mode:
                ...
                if profile.sentinel_mode:
                    master = conn.client.sentinel_master
                    replica_count = len(conn.client.sentinel_replicas)
                    self.set_status(
                        f"Connected via Sentinel to {master[0]}:{master[1]} "
                        f"({replica_count} replicas, reads: {profile.read_preference})",
                        'success'
                    )
                else:
                    self.set_status(f"Connected to {profile.host}:{profile.port}", 'success')

            self.interface_mode = 'main'
            self.loop.widget = self.main_frame
            self.left_panel.set_title(f'{profile.name} - Databases & Keys')
            self.main_frame.focus_position = 'body'
            self.main_content.focus_position = 1
            self.content_columns.focus_position = 0
            self.left_panel_pile.focus_position = 0
            self.update_connection_tree()
            self.refresh_keys()
            return True
        else:
            self.profile_status_text.set_text(
                ('error', f' Connection to {profile.name} failed')
            )
            self.show_connection_error(profile, conn, message)
            return False
    ...
    def disconnect(self):
        """Close the active connection and return to the profile selector."""
        self._cancel_active_scan()
        if self.current_connection:
            name = self.current_connection.profile.name
            cache_key = id(self.current_connection)
            try:
                self.current_connection.disconnect()
            except Exception as e:
                logger.error(f'Error while closing connection {name}: {e}')
            self.connections.pop(name, None)
            self._db_key_counts_cache.pop(cache_key, None)
            self._database_count_cache.pop(cache_key, None)
            self.current_connection = None
            self.current_key = None
            self.key_count = 0
            self.key_list_view.marked_keys.clear()
            self.key_list_view.begin_update()
            self.detail_walker[:] = []

            if self.console_mode:
                self.toggle_console()

            self.left_panel.set_title('Databases & Keys')
            self.update_connection_tree()
            self.interface_mode = 'profiles'
            self.profile_status_text.set_text(
                ('success', f' Disconnected from {name}. Select another profile.')
            )
            self.loop.widget = self.profile_frame
    ...
    def refresh_keys(self):
        """Start a cancellable background scan and update the list in batches."""
        if not self.current_connection or not self.current_connection.connected:
            self._cancel_active_scan()
            self.key_list_view.begin_update()
            return
        ...
        self.key_count = 0
        self.key_list_view.begin_update()
        self._invalidate_db_key_counts(self.current_connection)
        ...
        if self.current_connection.is_cluster:
            ...
            pools = [('standalone', client.main_pool)]

        self._scan_expected_workers = len(pools)
        if not pools:
            self._scan_active = False
            self.set_status("No Redis master nodes available for scanning", 'error')
            return

        if self.current_connection.is_cluster:
            mode = 'cluster'
        elif self.current_connection.is_sentinel:
            mode = 'sentinel master'
        else:
            mode = 'standalone'
        logger.info(f"Starting {mode} scan with {len(pools)} worker(s)")
        self.set_status(f"Scanning keys with {len(pools)} worker(s)...")
        scan_budget = {'lock': threading.Lock(), 'reserved': 0}

        for node_id, pool in pools:
            worker = threading.Thread(
                target=self._scan_pool_worker,
                args=(generation, node_id, pool, cancel_event, scan_budget),
                name=f"redis-scan-{node_id}",
                daemon=True,
            )
            worker.start()

        self.loop.set_alarm_in(0, self._poll_scan_results, generation)

    def _cancel_active_scan(self):
        """Signal the current scan workers to stop."""
        if self._scan_cancel_event:
            self._scan_cancel_event.set()
        self._scan_active = False

    def _scan_pool_worker(self, generation: int, node_id: str, pool,
                          cancel_event: threading.Event, scan_budget=None):
        """Scan one Redis node and publish typed key batches to the UI queue."""
        conn = None
        scanned_count = 0
        ...
        try:
            ...
            while not cancel_event.is_set() and scanned_count < self.MAX_KEYS:
                response = conn.execute_command(
                    'SCAN', cursor, 'MATCH', '*', 'COUNT', self.SCAN_COUNT
                )
                cursor = int(response[0])
                keys = response[1]

                for offset in range(0, len(keys), self.TYPE_PIPELINE_BATCH_SIZE):
                    if cancel_event.is_set() or scanned_count >= self.MAX_KEYS:
                        break

                    key_batch = keys[offset:offset + self.TYPE_PIPELINE_BATCH_SIZE]
                    with scan_budget['lock']:
                        remaining = self.MAX_KEYS - scan_budget['reserved']
                        if remaining <= 0:
                            cancel_event.set()
                            break
                        key_batch = key_batch[:remaining]
                        scan_budget['reserved'] += len(key_batch)
                        budget_exhausted = (
                            scan_budget['reserved'] >= self.MAX_KEYS
                        )
                    commands = [('TYPE', key) for key in key_batch]

                    try:
                        type_results = conn.execute_pipeline(commands)
                    except Exception as e:
                        type_results = ['unknown'] * len(key_batch)
                        self._scan_queue.put(('error', generation, node_id, str(e)))
                    ...
                    scanned_count += len(keys_with_types)
                    self._scan_queue.put(
                        ('batch', generation, node_id, keys_with_types)
                    )
                    if budget_exhausted:
                        cancel_event.set()
            ...
            logger.error(f"Error scanning node {node_id}: {e}", exc_info=True)
            self._scan_queue.put(('error', generation, node_id, str(e)))
        finally:
            if conn is not None:
                pool.release(conn)
            self._scan_queue.put(('worker_done', generation, node_id, scanned_count))

    def _poll_scan_results(self, loop, generation: int):
        """Move background scan results into Urwid widgets on the main thread."""
        if generation != self._scan_generation or not self._scan_active:
            return
        ...
        while processed < 8:
            ...
            processed += 1
            event_type, event_generation, node_id, payload = event
            if event_generation != generation:
                continue

            if event_type == 'batch':
                remaining = self.MAX_KEYS - len(self._scan_seen_keys)
                unique_batch = []

                for key, key_type in payload:
                    if remaining <= 0:
                        break
                    if key in self._scan_seen_keys:
                        continue
                    self._scan_seen_keys.add(key)
                    unique_batch.append((key, key_type))
                    remaining -= 1

                if unique_batch:
                    self.key_list_view.append_keys(unique_batch)
                    self.key_count = len(self._scan_seen_keys)
                    self.set_status(f"Scanning... {self.key_count} keys")

                if self.key_count >= self.MAX_KEYS:
                    if self._scan_cancel_event:
                        self._scan_cancel_event.set()
                    self._finish_key_scan(limit_reached=True)
                    return

            elif event_type == 'error':
                self._scan_errors.append(f"{node_id}: {payload}")

            elif event_type == 'worker_done':
                self._scan_finished_workers += 1

        if self._scan_finished_workers >= self._scan_expected_workers:
            self._finish_key_scan(limit_reached=False)
            return

        loop.set_alarm_in(0.05, self._poll_scan_results, generation)

    def _finish_key_scan(self, limit_reached: bool):
        """Finalize an incremental scan and refresh derived UI state once."""
        ...
        if (
                self._scan_errors
                and self.current_connection
                and self.current_connection.is_sentinel
        ):
            try:
                self.current_connection.client.refresh_sentinel_topology()
            except Exception as e:
                logger.warning(f'Sentinel refresh after scan failure failed: {e}')
        self.update_connection_tree()

        suffix = " (limit reached)" if limit_reached else ""
        if self._scan_errors:
            self.set_status(
                f"Loaded {self.key_count} keys{suffix}; "
                f"{len(self._scan_errors)} scan error(s)",
                'error'
            )
        else:
            self.set_status(f"Loaded {self.key_count} keys{suffix}", 'success')

        if self.current_key and self.current_key in self._scan_seen_keys:
            self.display_key_details(self.current_key)
        logger.info(f"Key scan complete: {self.key_count} keys{suffix}")
    ...
    def update_tabs(self):
        ...
        if self.current_connection and self.current_connection.connected:
            ...
            if self.current_connection.is_cluster:
                label = f" ● {name} [Cluster, DB0] "
                tabs.append(('tab_active', label))
            elif getattr(self.current_connection, 'is_sentinel', False):
                db = self.current_connection.current_db
                master = self.current_connection.client.sentinel_master
                label = (
                    f" ● {name} [Sentinel {master[0]}:{master[1]}, DB{db}] "
                )
                tabs.append(('tab_active', label))
            else:
                db = self.current_connection.current_db
    ...
    def _cycle_main_panel_focus(self, direction: int):
        """Cycle focus across databases, keys, and key details."""
        if self.content_columns.focus_position == 1:
            current_index = 2
        elif self.left_panel_pile.focus_position == 2:
            current_index = 1
        else:
            current_index = 0

        panel_names = ('Databases', 'Keys', 'Details')
        next_index = (current_index + direction) % len(panel_names)
        ...
        if next_index == 0:
            ...
            self.content_columns.focus_position = 0
            self.left_panel_pile.focus_position = 2
        else:
            self.content_columns.focus_position = 1

        self.set_status(f"Focus: {panel_names[next_index]}")

    def handle_input(self, key):
        """Dispatch global shortcuts for the selector, modal, and main UI."""
        if key == 'f10':
            self.exit_main()
        elif key in ('q', 'Q') and not self.console_mode:
            self.exit_main()
        elif getattr(self, 'interface_mode', 'main') == 'connection_error':
            if key == 'esc':
                self.close_connection_error()
                return True
            return
        elif getattr(self, 'interface_mode', 'main') != 'main':
            return

        if key == 'tab':
            self._cycle_main_panel_focus(1)
            return True
        elif key in ('shift tab', 'shift-tab', 'backtab'):
            self._cycle_main_panel_focus(-1)
            return True
...
Hotkeys:
  ...
  Tab        - Focus servers → keys → details
  Shift+Tab  - Focus details → keys → servers
  /          - Key Filter
  Ctrl+a     - mark keys
  Ctrl+u     - unmark keys 
  Enter      - View key
